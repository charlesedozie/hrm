...
@ApiTags('InterventionCategory')
@ApiBearerAuth()
@Controller('intervention-category')
@UseGuards(JwtAuthGuard)
export class InterventionCategoryController {
  constructor(private readonly service: InterventionCategoryService) {}

  @Post()
create(
  @Body() dto: CreateInterventionCategoryDto,
  //@Body() body: any,
  ...
  try {
    const dto = new CreateInterventionCategoryDto();
  } catch (e) {
  ...
  @Put(':id')
  update(@Param('id') id: string, @Body() data: UpdateInterventionCategoryDto): Promise<InterventionCategory> {
    return this.service.update(id, data);
  }

  @Delete(':id')
  remove(@Param('id') id: string): Promise<InterventionCategory> {
    return this.service.delete(id);
  }


  @Get()
findAll(@Query() query: QueryInterventionCategoryDto) {
