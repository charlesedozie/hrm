...
create(
  ...
  @Put(':id')
  update(@Param('id') id: string, @Body() data: UpdateLeaveTypeDto): Promise<LeaveType> {
...
findAll(@Query() query: QueryLeaveTypeDto) {
  ...
  @Get('balances')
findAllBalance(@Query() query: QueryLeaveTypeDto) {
  return this.service.getAllBalance(query); }
