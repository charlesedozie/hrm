...
@ApiTags('OfficeRole')
@ApiBearerAuth()
@Controller('officerole')
@UseGuards(JwtAuthGuard)
export class OfficeRoleController {
  constructor(private readonly service: OfficeRoleService) {}

  @Post()
create(
  @Body() dto: CreateOfficeRoleDto,
  //@Body() body: any,
  ...
  try {
    const dto = new CreateOfficeRoleDto();
  } catch (e) {
  ...
  @Put(':id')
  update(@Param('id') id: string, @Body() data: UpdateOfficeRoleDto): Promise<OfficeRole> {
    return this.service.update(id, data);
  }

  @Delete(':id')
  remove(@Param('id') id: string): Promise<OfficeRole> {
    return this.service.delete(id);
  }


  @Get()
findAll(@Query() query: QueryOfficeRoleDto) {
