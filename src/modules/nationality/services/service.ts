...
export class NationalityService {
...
): Promise<{ items: Nationality[]; pagination: any }> {
  const {
    page = 1,
  ...
  if (id) {
    const item = await this.prisma.nationality.findUnique({ where: { id } });
  ...
  const total = await this.prisma.nationality.count({ where });

  let items: Nationality[];
  let pagination: any;
  ...
  if (wantsAll) {
    ...
    items = await this.prisma.nationality.findMany({
      where,
      orderBy: {
        [sortBy]: sortOrder,
      },
  ...
  async getById(id: string): Promise<Nationality> { 
  const item = await this.prisma.nationality.findUnique({
    where: { id },
  });
